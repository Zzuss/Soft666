...
set "TOMCAT_PORT_DEFAULT=8080"
set "APP_NAME=ta-recruitment"
set "APP_URL=http://localhost:%TOMCAT_PORT_DEFAULT%/%APP_NAME%/"
...
:resolve_tomcat_port
if defined TOMCAT_PORT if not "%TOMCAT_PORT%"=="" (
    call :refresh_app_url
    exit /b 0
)

for /f "usebackq delims=" %%P in (`powershell -NoProfile -ExecutionPolicy Bypass -Command "$ports = 8080,8081,8082,8083; $busy = @(Get-NetTCPConnection -State Listen -ErrorAction SilentlyContinue | Select-Object -ExpandProperty LocalPort -Unique); ($ports | Where-Object { $busy -notcontains $_ } | Select-Object -First 1)"`) do (
    set "TOMCAT_PORT=%%P"
)

if not defined TOMCAT_PORT set "TOMCAT_PORT=%TOMCAT_PORT_DEFAULT%"
call :refresh_app_url
exit /b 0

:refresh_app_url
set "APP_URL=http://localhost:%TOMCAT_PORT%/%APP_NAME%/"
exit /b 0
...
call :configure_tomcat_port
call :prepare_tomcat_env
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$serverXml = Join-Path '!TOMCAT_HOME!' 'conf\server.xml'; $content = Get-Content $serverXml -Raw; $updated = $content.Replace('port=""8080"" protocol=""HTTP/1.1""', 'port=""!TOMCAT_PORT!"" protocol=""HTTP/1.1""'); if ($updated -ne $content) { Set-Content -Path $serverXml -Value $updated -Encoding UTF8 }"
exit /b 0
